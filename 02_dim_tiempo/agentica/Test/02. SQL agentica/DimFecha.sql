/* ============================================================================
   ...
   Warehouse). Generado por dim-tiempo-casi-perfecta a partir de
   ../../../scripts/04_tabla_fecha_tsql.sql v2.

   Notas de diseño frente a la versión Power Query:
   - La columna [Date] se genera como DATE (no DATETIME).
   - DayOffset/MonthOffset/QuarterOffset usan DATEDIFF, que cuenta límites de
     calendario cruzados, no restas de tiempo.
   - IsoWeek usa DATEPART(ISO_WEEK, ...), ISO 8601 nativo desde SQL Server 2012.
   - FiscalYear/FiscalQuarter se calculan directamente desde Year/Month y
     @FiscalStartMonth, con @FiscalYearBasis decidiendo explícitamente si el
     año fiscal se nombra por el año en que arranca o por el año en que
     termina.
   - IsWorkingDay marca de lunes a viernes; no contempla festivos (resuélvelo
     aparte, cruzando contra tu propia tabla de festivos). WorkingDayNumber es
     un contador de días laborables acumulado dentro de cada año natural.
   ========================================================================== */
...
DECLARE @StartYear         INT          = 2021;
DECLARE @EndYear           INT          = 2028;
DECLARE @FiscalStartMonth  INT          = 6;       -- 6 = mes en que arranca el año fiscal
DECLARE @FiscalYearBasis   VARCHAR(5)   = 'Start';   -- 'Start' o 'End' — ver cabecera
...
Fiscal AS (
    SELECT
        ...
        -- Orden cronológico dentro del año fiscal (1 = mes de arranque fiscal,
        -- 12 = el mes justo anterior); necesario para que un informe que
        -- agrupe por FiscalYear muestre los meses en orden Jun..May (con
        -- @FiscalStartMonth=6) en vez de Ene..Dic. Mismo patrón de doble %
        -- que FiscalQuarter, por la misma razón: el % de T-SQL da el resto
        -- con el signo del dividendo, no un módulo siempre-positivo.
        ((((b.[Month] - @FiscalStartMonth) % 12) + 12) % 12) + 1              AS FiscalMonthNumber
    FROM Base b
),
Enriched AS (
    SELECT
        f.*,
        f.[Month Name]                                                       AS [Fiscal Month Name],
...
-- SELECT ... INTO: patrón soportado por SQL Server on-prem/Azure SQL para
-- materializar una consulta como tabla (CREATE TABLE ... AS SELECT, el CTAS
-- de Fabric Warehouse, no existe en SQL Server estándar).
SELECT
    n.*,
